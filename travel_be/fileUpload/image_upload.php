...
if(isset($_POST["f_image_upload_button"])){
	$stmt1 = $conn->prepare("INSERT INTO table_file (file_name,file_path,file_uploader,file_type) VALUES (?,?,?,'image')");
	...
	$image_form = $_FILES["form_image"];
	$image_count = count($image_form["tmp_name"]);
	for($i = 0;$i < $image_count;$i++){
		$extension = pathinfo($image_form["name"][$i],PATHINFO_EXTENSION);
		$f_path = upload_move_file("./../travel_fe/images/",$image_form["tmp_name"][$i],$extension);
